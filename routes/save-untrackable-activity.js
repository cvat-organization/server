...
router.put('/', authMiddleware, async(req, res) => {
    try {
        ...
        // Store all the fields in the untrackable activites' history array
        const result = await users.findOneAndUpdate(
            { _id },
            { $push: { untrackableActivitesHistory: { activityName, subActivity, startTime, endTime, parameters, comments } } },
            { new: true }
        );
        
        // Respond to the client w appropriate message
        if (result) {
            const activityHistoryID = result.untrackableActivitesHistory[result.untrackableActivitesHistory.length - 1]._id;
